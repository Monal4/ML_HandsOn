...
MeanShiftClusters = cluster.MeanShift()
MeanShiftClusters.fit(pca_features)
print(len(pca_features))
print(MeanShiftClusters.cluster_centers_)
print(MeanShiftClusters.labels_)

plt.scatter(
    MeanShiftClusters.cluster_centers_[:, 0],
    MeanShiftClusters.cluster_centers_[:, 1]
)
...
for i in range(len(pca_features)):
    plt.plot(
        pca_features[i][0],
        pca_features[i][1],
        ['rx', 'gx', 'yx'][MeanShiftClusters.labels_[i]]
    )
