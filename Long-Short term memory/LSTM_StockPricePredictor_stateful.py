...
class Network:
    def __init__(self):
        ...
        # LSTM with stateful hidden and cell states -- Not recommended due to overlapping batches
        self.hidden_state = np.random.randn(32, 1)
        self.cell_state = np.random.randn(32, 1)
    ...
    def LSTM(self, training_features, learning_rate, batch_size, testing_features):
        ...
        # print("Cell state before : ", self.cell_state)
        # print("Hidden state before : ", self.hidden_state)
        ...
        for cells, result in batches:
            backprop_cell_state, backprop_hidden_state, biases_backprop, weights_backprop = self.feed_forward_and_back_prop_batch(
                cells, result)
            counter += 1
            ...
            self.hidden_state = self.hidden_state - (learning_rate * backprop_hidden_state) / 60
            self.cell_state = self.cell_state - (learning_rate * backprop_cell_state) / 60
            if counter == 10979:
                # print("weights after : ", self.weights)
                # print("Biases after : ", self.biases)
                # print("Cell state after : ", self.cell_state)
                # print("Hidden state after : ", self.hidden_state)
                print("Break point")
    ...
    def feed_forward_and_back_prop_batch(self, cells, result):
        ...
        backprop_cell_state, backprop_hidden_state = self.backprop(activations, biases_backprop, cell_states, cells,
                                                                   final_activation, result, weights_backprop, z1s, z2s,
                                                                   z3s, z4s)
        return backprop_cell_state, backprop_hidden_state, biases_backprop, weights_backprop
